...
class PageManager {
    ...
    renderMenuPage() {
        ...
        let menuHTML = `
            <div id="menu-content" class="container mx-auto px-4 py-8 relative z-10">
                <div class="text-center mb-8">
                    <h1 class="text-4xl font-bold text-white mb-2 drop-shadow-lg">Our Menu</h1>
                    <p class="text-lg text-white drop-shadow-md">Out of this world pies and pastries at Jupiter's Pies</p>
                </div>
                
                <nav class="mb-8">
                    <div class="overflow-x-auto">
                        <div class="flex space-x-4 pb-2 bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-lg">
                            ${menuSections.map(section => 
                                `<a href="#${section.replace(/\s+/g, '-').toLowerCase()}" 
                                    class="px-4 py-2 border-b-2 border-b-transparent text-gray-700 font-medium hover:text-orange-600 hover:border-orange-600 whitespace-nowrap transition-colors">
                                    ${section}
                                </a>`
                            ).join('')}
                        </div>
        ...
        menuSections.forEach(section => {
            ...
            menuHTML += `
                <div id="${sectionId}" class="mb-16 scroll-mt-16 menu-category">
                    <h2 class="text-3xl font-bold text-white mb-8 pb-2 border-b border-white/30 drop-shadow-lg">${section}</h2>
    ...
    getFoodEmoji(name) {
        ...
        // Jupiter's Pies specific items
        if (nameLower.includes('pie')) {
            if (nameLower.includes('apple')) return '🍎';
            if (nameLower.includes('cherry')) return '🍒';
            if (nameLower.includes('berry') || nameLower.includes('blueberry') || nameLower.includes('strawberry')) return '🫐';
            if (nameLower.includes('pumpkin')) return '🎃';
            if (nameLower.includes('pecan')) return '🌰';
            if (nameLower.includes('chocolate')) return '🍫';
            if (nameLower.includes('key lime') || nameLower.includes('lime')) return '🍋';
            if (nameLower.includes('coconut')) return '🥥';
            if (nameLower.includes('sweet potato')) return '🍠';
            if (nameLower.includes('tomato')) return '🍅';
            return '🥧'; // Default pie emoji
        }
        ...
        if (nameLower.includes('roll') || nameLower.includes('cinnamon')) return '🥐';
        if (nameLower.includes('danish')) return '🧁';
        if (nameLower.includes('turnover')) return '🥧';
        ...
        if (nameLower.includes('fruit') || nameLower.includes('berry')) return '🍓';
        ...
        return `
            <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-full flex flex-col">
                <div class="h-48 bg-gray-100 flex items-center justify-center">
                    <span class="text-6xl">${this.getFoodEmoji(item.name)}</span>
                </div>
                <div class="p-5 flex-1 flex flex-col">
                    ...
                    <p class="text-black mb-4 flex-1">${item.description}</p>
                    
                    <button class="add-to-cart-btn w-full bg-orange-600 hover:bg-orange-700 text-white font-medium py-2 px-4 rounded-lg transition-colors"
